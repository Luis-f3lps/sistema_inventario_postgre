...
async function inicializarMenuEAutenticacao() {
  ...
  if (menuContainer) {
    try {
      ...
      if (response.ok) {
        ...
        menuContainer.innerHTML =
          '<p style="color: red; text-align: center;">Erro: menu.html não pôde ser carregado.</p>';
      }
    } catch (error) {
    ...
    console.error(
      'O elemento <div id="menu-container"></div> não foi encontrado no seu HTML principal.',
    );
    return;
  }
  ...
  document.dispatchEvent(
    new CustomEvent("menuReady", { detail: { userData } }),
  );
}
...
function ativarFuncionalidadeMenu() {
  ...
  if (openMenuBtn)
    openMenuBtn.addEventListener("click", () => (navMenu.style.left = "0px"));
  if (closeMenuBtn)
...
function preencherDadosDoMenu(userData) {
  ...
  if (userNameElement) {
    userNameElement.textContent =
      userData.nome_usuario || userData.nome || "Usuário";
  }
