...
const salaEl = document.getElementById('salas-select2');
...
async function loadSalas() {
    try {
        // Puxa as salas da rota que criamos no app.js
        const response = await fetch('/api/salas');
        const data = await response.json();
        
        salaEl.innerHTML = '<option value="">Selecione uma Sala</option>'; 
        data.forEach(sala => {
            const option = document.createElement('option');
            option.value = sala.id_sala;
            option.textContent = sala.nome_sala;
            salaEl.appendChild(option);
        });
    } catch (error) {
...
async function submeterAgendamentoRecorrente() {
    ...
    if (!salaEl.value) return alert("Selecione uma sala de aula.");
    ...
    if (precisaTecnico && linkRoteiro === '') {
        return alert("Por favor, insira o link do material. Ele é obrigatório quando o apoio técnico é solicitado.");
    }

    // Montamos o Payload enviando o ID da Sala
    const payload = {
        salaId: parseInt(salaEl.value),
    ...
    try {
        // ATENÇÃO: Apontando para uma nova rota específica de Salas no Backend!
        const response = await fetch('/api/schedule-recurring-salas', {
            method: 'POST',
        ...
        msgEl.textContent = "Reserva de salas solicitada com sucesso!";
