...
app.get("/api/aulas-hoje-tecnico", Autenticado, async (req, res) => {
  try {
    ...
    console.error("Erro ao buscar aulas de hoje tecnico:", err);
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT 
       ...
       JOIN laboratorio l ON a.id_laboratorio = l.id_laboratorio 
       JOIN horarios h ON a.id_horario = h.id_horario
       LEFT JOIN laboratorio_usuario lu ON l.id_laboratorio = lu.id_laboratorio -- CORREÇÃO AQUI
       WHERE 
         a.professor_email = $1 
         OR (a.precisa_tecnico = true AND lu.usuario_email = $1)`,
      [email],
    );
    res.json(result.rows);
  } catch (err) {
...
app.get("/api/relatorio-aulas-pdf", Autenticado, async (req, res) => {
  try {
    ...
    const pos = { id: 30, prof: 60, disc: 185, lab: 310, resp: 435, data: 560, horaIn: 615, horaFim: 650, alunos: 685, tec: 720, status: 750 };
    const wid = { prof: 120, disc: 120, lab: 120, resp: 120 }; 
